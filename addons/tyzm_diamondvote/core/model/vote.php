		...
		if ($voteinsertid) {
			...
			if (pdo_query($setvotesql)) {
				...
				if (empty($reply['isvotemsg'])) {
					$uservoteurl = $_W['siteroot'] . 'app/' . $this->createMobileUrl("view", array("rid" => $rid, "id" => $id));

					$content = '您的好友给你（' . $voteuser['noid'] . '）号【' . $voteuser['name'] . '】投了一票！目前' . ($voteuser['votenum'] + 1) . '票。<a href=\"' . $uservoteurl . '\">点击查看详情<\/a>';
